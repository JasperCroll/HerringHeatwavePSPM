/***
   ...
   PURPOSE
	 ...
	 This version of the model stops at every jump in the varaible derivatives and is therefore very slow.
  ...
  
{
	...
	double Eg, Ea, Em;
	double IzL, Ib, BlB, hand, etab, etazL, Az, Ab;
	double temp = gettemp(time) + getheatwave(time);
	...
	for (int i = 0; i < cohort_no[0]; i++) {
		...
		// transition larvea to juveniles
		if(fabs(pop[herring][age][i]) < fabs(events[0])) events[0] = pop[herring][age][i]; 


		
		// maturation
		if(fabs(len-LM) < fabs(events[1])) events[1] = len - LM;
		
		// start of fisheries
		if (fabs(len - LFMIN) < fabs(events[2])) events[2] = len - LFMIN;
		
		// starvation mortality
		if (fabs(cond - QS) < fabs(events[3])) events[3] = cond - QS;
		...
		if (fabs(Eg) < fabs(events[4])) events[4] = Eg;
		
		//printf("calc switch coho %.12f %.12f\n", pop[herring][age][i], events[0]);
	}
	// printf("calc switch %.12f  %.12f  %.12f  %.12f  %.12f  %.12f\n", events[0], events[1], events[2], events[3], events[4]);
